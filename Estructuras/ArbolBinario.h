...
#ifndef PROYECTOFINAL_PROGRAMACION_3_ARBOLBINARIO_H
#define PROYECTOFINAL_PROGRAMACION_3_ARBOLBINARIO_H
...
private:
    ...
    int contarPorNivel(NodoArbol<T> *nodo, int nivel);
};
/**
...
int ArbolBinario<T>::contarPorNivel(int nivel) {
    return contarPorNivel(root, nivel);
}

template <class T>
int ArbolBinario<T>::contarPorNivel(NodoArbol<T> *nodo, int nivel) {
    if (nodo == nullptr) {
        return 0;
    }
    if (nivel == 0) {
        ...
        return contarPorNivel(nodo->getLeft(), nivel - 1) + contarPorNivel(nodo->getRight(), nivel - 1);
    }
}



#endif //PROYECTOFINAL_PROGRAMACION_3_ARBOLBINARIO_H
